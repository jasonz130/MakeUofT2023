...
void loop() {
  ...
  if (MIC_COUNTER < MIC_NUM_OF_CALIBRATIONS){
    MIC_ANALOG_VAL = analogRead(MIC_ANALOG_PIN);
    MIC_RUNNING_SUM = MIC_RUNNING_SUM + (MIC_ANALOG_VAL / 100.);
    MIC_COUNTER++;
    Serial.print(str1);
    Serial.println(MIC_RUNNING_SUM);
    Serial.print(str3);
    Serial.println(MIC_NUM_OF_CALIBRATIONS);
    Serial.print(str4);
    ...
    if (MIC_RUNNING_SUM_CALC == 0){
      MIC_CALIBRATED_VAL = (MIC_RUNNING_SUM * 100) / MIC_NUM_OF_CALIBRATIONS;
      MIC_RUNNING_SUM_CALC = 1;
      digitalWrite(LED_PIN_CALIBRATE, HIGH);
      Serial.print(str5);
      Serial.println(MIC_CALIBRATED_VAL);
      delay(3000);
      digitalWrite(LED_PIN_CALIBRATE, LOW);
    }
    
    MIC_ANALOG_VAL = analogRead(MIC_ANALOG_PIN);
    Serial.print(str2);
    Serial.println(MIC_ANALOG_VAL);
    if ((MIC_ANALOG_VAL > (MIC_CALIBRATED_VAL + MIC_CALIBRATED_VAL_MODIFIER)) || (MIC_ANALOG_VAL < (MIC_CALIBRATED_VAL - MIC_CALIBRATED_VAL_MODIFIER))){
      Serial.println("AYYAAYYAYAYAYAAYAYYA");
      MICROPHONE_SIGNAL = 1;
